...
// addMaterial(name as string, color as string, parts as string[], moltenStats as int[]);
...
//      - luminosity
//      - viscosity
function addMaterial(name as string, color as string, parts as string[], moltenStats as int[]) {
    var acolor = Color.fromHex(color) as Color;
    var mat = MaterialSystem.getMaterialBuilder().setName(name).setColor(acolor).build();
    
    for part in parts {
        var d = mat.registerPart(part).getData();
