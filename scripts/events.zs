...
events.onEntityLivingDeathDrops(function(event as crafttweaker.event.EntityLivingDeathDropsEvent) {
	...
	for i in 0 to drops.length {
		if (drops[i].item.definition.id == <minecraft:skull:1>.definition.id && drops[i].item.name has "Wither Skeleton Skull") {
...
events.onBlockHarvestDrops(function(event as crafttweaker.event.BlockHarvestDropsEvent) {
	...
	if (event.block has <block:minecraft:skull:1>.getBlock()) {
		var drops = event.drops as IEntityItem[];
		if (drops.length == 0) {
			return;
		}
		for i in 0 to drops.length {
			if (drops[i].item.definition.id == <minecraft:skull:1>.definition.id && drops[i].item.name has "Wither Skeleton Skull") {
				drops[i] = <minecraft:skull:3>.withTag({SkullOwner: {Id: "7ed571a5-9fb8-416c-8b9d-fb2f446ab5b2", Properties: {textures: [{Signature: "", Value: ""}]}, Name: "Test_MHF_WSkeleton"}}).createEntityItem(event.world, event.position);
			}
		}
	}
});
