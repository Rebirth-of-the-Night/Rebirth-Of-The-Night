...
for i, subitem in <iceandfire:dragon_skull>.definition.subItems {
	recipes.addShapeless("dragon_head_smashable_"~i, subitem, 
		[subitem.marked("toRemove")],
		function(out, ins, cInfo) {
            var d = D(ins.toRemove.tag);
			return subitem.withTag({Stage:d.getInt("Stage", 1)});
		},
		null
	);
	for j in 1 to 6 {
		GraniteAnvil.addRecipe("DragonHead_"~i~"-"~j, <mod_lavacow:sharptooth>*((0.5*j*j+2.5*j) as int), subitem.withTag({Stage: j}),3*j,"hammer", true);
	}
}
